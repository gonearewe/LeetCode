...
object Solution {
  def reorderSpaces(text: String): String = {
    val cnt = text.count(_ == ' ')
    val li = text.trim.split("\\s+")
    if (li.size == 1) {
      val a = new StringBuilder(li(0))
      (0 until text.length - a.length).foreach(i => a append ' ')
      return a.toString()
    }
    val n = cnt / (li.size - 1)

    val a = new StringBuilder(text.length)
    li foreach { s =>
      a append s
      (0 until n).foreach(i => a append ' ')
    }
    a.delete(a.length - n, a.length)
    (0 until text.length - a.length).foreach(i => a append ' ')
    a.toString()
  }
}