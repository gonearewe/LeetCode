...
object Solution {
  def maxSumRangeQuery(nums: Array[Int], requests: Array[Array[Int]]): Int = {
    val m = 1000000007
    val n = nums.length
    val cnts = new Array[Int](n)
    requests foreach { case Array(start, end) =>
    ...
    val res = nums.clone()
    (cnts.zipWithIndex.sorted.map { e => e._1 } zip nums.sorted).foldLeft(0) { (sum, e) =>
      e match {
        case (cnt, num) =>
          (sum % m + (cnt % m * num % m) % m) % m // 利用同余定理取余
      }
    }
  }
}